...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        ...
        $coberturas = $coberturaController->viewCobertura();
    }
...
<body>
    ...
    <script>
                    ...
                    success: function (data) {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(function (cobertura) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + cobertura.descCobertura + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + cobertura.idCobertura + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + cobertura.idCobertura + ',\'' + cobertura.descCobertura + '\')">Editar</button></td>');
        ...
        function confirmarEdicao(idCobertura, descCobertura) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-cobertura.php?id=${idCobertura}&desc=${descCobertura}`;
