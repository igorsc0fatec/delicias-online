...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        ...
        $recheios = $recheioController->viewRecheio();
    }
...
<body>
        ...
        <form id="search-form" method="post">
            <div class="pesquisa">
                ...
                <div class="input-wrapper">
                    <input id="pesq" type="text" name="pesq" placeholder="Digite o nome do recheio/ex: doce de leite"
                        required>
    ...
    <script>
                    ...
                    success: function (data) {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(function (recheio) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + recheio.descRecheio + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + recheio.idRecheio + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + recheio.idRecheio + ',\'' + recheio.descRecheio + '\')">Editar</button></td>');
        ...
        function confirmarEdicao(idRecheio, descRecheio) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-recheio.php?id=${idRecheio}&desc=${descRecheio}`;
