...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        ...
        $massas = $massaController->viewMassa();
    }
...
<body>
        ...
        if ($massaController->verificaMassa()) {
            echo "
                <script>
                    ...
                    text: 'Essa massa já esta cadastrado!',
                    icon: 'error',
        ...
        <form id="search-form" method="post">
            <div class="pesquisa">
                ...
                <div class="input-wrapper">
                    <input id="pesq" type="text" name="pesq" placeholder="Digite o nome da massa/ex: pão de ló"
                        required>
    ...
    <script>
                    ...
                    success: function (data) {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(function (massa) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + massa.descMassa + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + massa.idMassa + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + massa.idMassa + ',\'' + massa.descMassa + '\')">Editar</button></td>');
        ...
        function confirmarEdicao(idMassa, descMassa) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-massa.php?id=${idMassa}&desc=${descMassa}`;
