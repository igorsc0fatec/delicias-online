...
if (isset($_GET['action']) && $_GET['action'] == 'fetch_data') {
    ...
    if (isset($_POST['pesq']) && !empty($_POST['pesq'])) {
        ...
        $formatos = $formatoController->viewFormato();
    }
...
<body>
    ...
    <script>
                    ...
                    success: function (data) {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(function (formato) {
                                var row = $('<tr></tr>');
                                row.append('<td>' + formato.descFormato + '</td>');
                                row.append('<td><button onclick="confirmarExclusao(' + formato.idFormato + ')">Excluir</button></td>');
                                row.append('<td><button onclick="confirmarEdicao(' + formato.idFormato + ',\'' + formato.descFormato + '\')">Editar</button></td>');
        ...
        function confirmarEdicao(idFormato, descFormato) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `editar-formato.php?id=${idFormato}&desc=${descFormato}`;
