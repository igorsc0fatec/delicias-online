...
$tipoTelefone = $clienteController->viewTipoTelefone();
...
if (isset($_GET['id'])) {
    header('Content-Type: application/json');
    echo json_encode($clienteController->deleteTelefone($_GET['id']));
    exit;
}
?>
<!DOCTYPE html>
...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        } else if ($clienteController->countTelefone()) {
            echo "
                <script>
                    Swal.fire({
    ...
    <div>
        ...
        <script>
                        ...
                        success: function (data) {
                            ...
                            if (data.length === 0) {
                                ...
                                data.forEach(function (telefone) {
                                    var row = $('<tr></tr>');
                                    row.append('<td>' + telefone.numTelCliente + '</td>');
                                    row.append('<td>' + telefone.tipoTelefone + '</td>');
                                    row.append('<td><button onclick="confirmarExclusao(' + telefone.idTelCliente + ')">Excluir</button></td>');
            ...
            function confirmarExclusao(idTelCliente) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: 'cadastrar-telefone-cliente.php?id=' + idTelCliente,
                            type: 'POST',
                            data: { idTelCliente: idTelCliente },
